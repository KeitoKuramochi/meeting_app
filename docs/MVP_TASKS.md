...
**完了条件**:
- [x] `/request/[存在するfarm_contact_id]` にアクセスすると相手の名前（先生名）が表示される
- [x] 「お名前」「相談内容」入力欄がある
- [x] 候補日時の入力欄がある（最低1件〜最大5件）
- [x] 名前・相談内容が空の状態で送信するとエラーメッセージが表示される
- [x] 候補日が0件の状態で送信するとエラーメッセージが表示される
- [x] 正しく入力して送信すると Supabase の `meetings` に `farm_contact_id` 付きでレコードが保存される
- [x] 送信完了後に `/r/[id]` 形式のURLが表示され、「URLをコピー」できる
- [x] `/request/[存在しないfarm_contact_id]` にアクセスするとエラーメッセージが表示される
- [x] スマホ幅（375px）でレイアウトが崩れない
- [x] `npm run build` がエラーなく通る
- [x] `git commit` が1件作成されている
...
**完了条件**:
- [x] `/farm` を開くとキャラクターが農園内を動き続けている（CSS animation ループ）
- [x] 各キャラの動きが互いに異なる（同じ動きにならない）
- [x] キャラクターが農園の表示領域外にはみ出さない
- [x] 確定回数 0 のキャラが scale 0.60 で ZZZ（a5.png）が表示される
- [x] 確定回数 3 のキャラにキラキラ（a2.png）が点滅表示される
- [x] 確定回数 4 以上のキャラにキラキラ＋ハート（a3.png）が交互表示される
- [x] 農園内で確定回数最多のキャラに王冠（a4.png）が表示される（1人でも表示）
- [x] 吹き出し（a1.png）がランダムタイミングでキャラ頭上に出て消える
- [x] 吹き出しの中にミーティングを促すランダムなセリフが表示される
- [x] キャラをタップすると `/request/[farm_contact_id]` へ遷移する
- [x] スマホ幅（375px）・PC幅（1280px）どちらでもレイアウトが崩れない
- [x] ダークモード（`prefers-color-scheme: dark`）でUI全体が正しく表示される
- [x] ボタンのタップ領域が44px以上
- [x] `npm run build` がエラーなく通る
- [x] `git commit` が1件作成されている

**実装メモ**: FarmContactWithCount 型を追加。page.tsx で meetings テーブルから confirmed_index IS NOT NULL の件数を JS 集計して各コンタクトに付与。FarmCharacters.tsx をフル書き換え：seed ベースの buildWalkParams でキャラごとに異なる周期・移動量・位相を生成し CSS keyframes ランダムウォーク実装。useBubble フックで 5〜15秒ランダムタイマーを管理。scale 0.60〜1.20 の成長表示、ZZZ/キラキラ/ハート交互（800ms interval）/王冠エフェクト実装。キャラクリックで router.push('/request/[id]') 遷移。SSR での Math.random() 使用を回避（sin ベースの deterministic seed 関数を使用）。
**commit hash**: b49037e
