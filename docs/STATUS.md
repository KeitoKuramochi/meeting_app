...
### 2026-07-07 調査・不具合修正 — 通知/確定フローの調査、Playwrightでの認証バイパス検証手法を確立

- ユーザーから「Playwrightでログインした風に仮想的に確認できるようにしてほしい」と依頼（Plan mode）。3つの選択肢を検討した結果、本物のOAuthセッションCookieを取得する方式ではなく、「ローカル検証中だけ`middleware.ts`・`farm/page.tsx`・`farm/add/page.tsx`の認証チェックを一時的にバイパスし、モックデータで表示する」方式で合意。実装→Playwrightで`/farm`・`/farm/add`を実機確認→`git checkout`で3ファイルを完全に元へ戻す、という一連の手順を確立し、再現可能な手順として`~/.claude/plans/squishy-tickling-barto.md`に記録した（このバイパスコードは都度追加・都度削除する運用で、リポジトリには残さない）
- ユーザーから「リンク作成→確定待ち表示→確定/別日提案→通知」の一連のフローを満遍なく調査・改善してほしいと依頼。3つのExploreエージェントを並行実行し、以下を発見:
  1. リンク作成→確定待ち表示は正常（吹き出しの`PendingDots`含め実装済み）
  2. **確定操作をしても、キャラの成長エフェクト・明示的な通知が一切出ない**。要約提出トリガーへの仕様変更（2026-07-05）の際、確定用の演出（LEVEL UP用a7.png、びっくり用a6.png）が未整理のまま放置されていた（a7.pngはどこからも参照されず未使用資産だった）
  3. **`/r/[id]`の確定判定漏れ**: `confirmed_index`のみを見ており、送信側の「手動確定」(`manually_confirmed`)を見ていなかったため、送信側が手動確定した後も教員側の`/r/[id]`は「提案を送りました」のロック画面から進めないバグがあった
  4. 「📬返信あり」表示が別日提案なのか他の返信なのか文言上区別できない（`replied_at`は別日提案フロー以外では絶対に書き込まれないことをgrepで確認済み）
  5. 別日提案された代替候補日を、アプリ内で選んで確定・再送信する手段がない（新規フロー追加相当の規模のため、ユーザーと相談の上、今回は見送り。将来のタスクとして残す）
  6. 「未送信ドラフトがあるまま返信も来た」場合、吹き出し文言・名前ラベル色・なかま一覧の3箇所で優先順位が食い違っていた
- ユーザーと相談し、⑤（代替候補の選択・再送信機能）は見送り、②③④⑥のバグ修正のみ今回実装することで合意
- 実装内容:
  - `FarmCharacters.tsx`: 確定回数が増えた瞬間に「🎉確定しました！」ポップ演出を追加（`showLevelUp`と同じ仕組みを`liveConfirmedCount`向けに複製、a7.png画像を活用）。吹き出し・名前ラベル・なかま一覧・履歴モーダルの優先順位を「別日提案 > 未送信 > 確定待ち」に統一。「返信あり」系の文言を「別日提案あり」に変更
  - `FarmClientShell.tsx`: なかま一覧の「📬 返信が届いています」を「📅 別の日程が提案されました」に変更
  - `r/[id]/page.tsx`: `isConfirmed`判定に`manually_confirmed === true`を追加
  - `r/[id]/CandidateList.tsx`: 確定済み表示の分岐を、`confirmed_index`が無い（手動確定のみの）ケースでも「確定しました」を表示できるよう変更（候補日の代わりに「送信者からの連絡内容で確定しています」と表示）
- `npm run build`成功、`npm run lint`は新規エラーなし（既存のpre-existingエラーのみ）
- Playwrightで実機確認できたこと（mock-authバイパスを一時適用→検証→即revert）: 「別日提案あり」文言、なかま一覧の新文言、`localStorage`で未送信ドラフトを注入しても優先順位が正しく保たれること
- **追記（同日・ユーザーから「ログイン機能を無くしてでも検証と評価と修正を繰り返してほしい」と依頼を受け、②③を本番DBへの書き込みなしで実機検証）**:
  - ③（`/r/[id]`の手動確定表示）: `r/[id]/page.tsx`に一時的なテスト用データ分岐（`PLAYWRIGHT_MOCK_AUTH`時のみ、DBには一切書き込まない）を追加し、4パターン（手動確定のみ／通常のconfirmed_index確定＝回帰確認／別日提案ロック画面＝回帰確認／別日提案→手動確定という実際のバグ再現シナリオ）をPlaywrightで実機確認。すべて意図通りの表示になることを確認し、テスト分岐は検証後に削除済み
  - ②（確定ポップ演出）: `FarmClientShell.tsx`に一時的なデバッグボタン（既存の`handleManualConfirmed`をそのまま呼ぶだけ）を追加してモック先生の確定回数を増やし、演出が発火することをアクセシビリティツリー・DOM直接検査（`getComputedStyle`）で確認。当初スクリーンショットに写らなかったのは演出時間が1.8秒のCSSアニメーション（既存の「成長した！」バッジと全く同じ`animate-badge-float`、fade-out込みで一瞬だけ表示される仕様）のため、Playwrightのツール呼び出し往復時間内に収まらなかっただけと判明（バグではない）。検証の過程で、確定バッジの表示位置が成長バッジより上にずれていて、キャラが農園の上端付近にいる時にクリップされる可能性がある実装をしていたため、成長バッジと同じ`bottom:100%`位置に統一する修正も追加した。デバッグボタンは検証後に削除済み
  - 上記の一時的なテスト分岐・デバッグボタンはすべて`git status`で差分ゼロになるまで削除済み。DBへの書き込み・認証設定の変更は一切行っていない
- `npm run build`・`npm run lint`再確認、新規エラーなし
- まだ`git commit`していない（ユーザーの明示的な指示待ち）
- 次にやること: 問題なければcommitの指示を出す。⑤（代替候補の選択・再送信機能）は将来のタスクとして別途検討する

### 2026-07-07 機能追加 — 「種をまく」にリンク作成である補足を追加、デモにプレビューモーダルを新設
- ユーザーから「種をまくとリンクを作るが繋がらないので括弧書きを追加してほしい」「デモで、リンクができること・相手にどう見えるか・どんな要約がされるかを見せてほしい。ガイドとデモ両方を更新し、デザインも綺麗にしてほしい」と依頼。「繰り返し評価と実装を、ゆっくり丁寧に」との指示も受けた
- 言い回しの補足（`FarmCharacters.tsx`のフッターボタン「🌱 新しい種をまく」→「🌱 新しい種をまく（リンクを作る）」、`RequestForm.tsx`の送信ボタン「🌱 種をまく」→「🌱 種をまく（リンクを作成）」、`guide/page.tsx`STEP3見出し・説明文）を追加
- 新規実装: `/demo`でキャラをタップした際に出る案内を、単なるトースト（「デモでは種をまけません」）から、実際の3ステップ（🌱種をまく＝リンク作成 → 🙋相手が見る画面 → 🌾肥料をあげる＝AI要約の例）を静的モックで見せる`DemoPreviewModal`（新規ファイル `src/app/demo/DemoPreviewModal.tsx`）に置き換え。既存の農園パレット（parchment/gold）やガイドページのモックUIパターンを踏襲し、`DemoFarmCharacters.tsx`のタップ処理・状態もそれに合わせて書き換え（`Toast`コンポーネントを削除し`previewContactName`state+モーダルに変更）
- `/code-review`スキル（medium）で自己レビューし、実際に直した指摘:
  - モックUI全体を`aria-hidden`にしていたため、見出しや説明文までスクリーンリーダーから読めなくなっていた不具合を修正（`aria-hidden`はモックUIの視覚的な再現部分だけに限定し、各ステップの見出し・新設した説明文はアクセシブルなまま残すよう再設計。副次的に、説明文が増えて晴眼者にとっても分かりやすくなった）
  - モックの「確定する」偽ボタンが本物の`.farm-btn`ホバー演出を持ったままで紛らわしかったため、`pointer-events: none`を付与し反応しないように修正
  - ステップ番号バッジのインラインスタイルが3箇所で重複していたため`StepBadge`ローカルコンポーネントに抽出
  - （対応不要と判断）ガイドページの既存モックUIとの重複自体は、2ページの役割が異なる（ガイド＝恒久的な説明、デモ＝体験）ため、共有コンポーネント化するほどの重複ではないと判断し見送り
- Playwright MCPで`/demo`（ログイン不要ページ）を実機確認。3ステップのプレビューが正しく表示・スクロールされることを確認済み
- `npm run build`成功、`npm run lint`は新規エラーなし
- 次にやること: 人間が実機で`/demo`のキャラタップ→プレビューモーダルの見た目・スクロールを確認 → 問題なければcommitの指示を出す
...
- 実装:
  - `src/lib/summarize.ts`: `summarizeWithGemini`をメイン、`summarizeWithClaude`をフォールバックに順序を入れ替え（プロンプト・モデル自体は変更なし）
  - `src/app/farm/FarmCharacters.tsx`: 要約編集フォームに「💾 内容だけ保存する（要約はそのまま）」ボタンを追加。文字起こし/メモ＋質問モードで、既存の要約は変更せず`summary_raw_input`/`summary_qa`（編集した下書き・回答）だけを保存できる（`handleSaveNotesOnly`関数を新設）。このボタンは「編集」時（既に要約が提出済みの記録）のみ表示され、初回提出時・貼り付けモードでは表示されない
  - 直近の要約表示（「前回のまとめ」）は既存の`latestSummaryItem`ロジックをそのまま使っており、編集後は`summary_submitted_at`が更新された記録が自動的に最上位に来るため追加対応は不要と判断
  - `npm run build`成功、`npm run lint`は新規エラーなし
- **要確認**: GeminiをメインAPIにしたことで、本番（Vercel）環境変数に`GEMINI_API_KEY`が設定されていないと要約生成が動かない（今まではフォールバックだったため`ANTHROPIC_API_KEY`だけでも動いていた）。過去のセッション履歴で「Vercel環境変数にGEMINI_API_KEYが未設定の可能性」と記録があるため、必ず確認してほしい
- 次にやること: 人間がVercelの環境変数`GEMINI_API_KEY`を確認 → 実機で「編集する」→「内容だけ保存」と「AIで要約を作り直す」の両方を試す

### 2026-07-06 機能追加 — 提出済み要約の編集機能
- ユーザーがメモ＋質問の回答に「a」とだけ入力して提出したところ、AIが「内容が確認できません」という趣旨の要約を返す挙動になり、ユーザーはこれを「良い挙動」と評価。その上で「振り返って入力しなおせたり、編集できたりする機能」を提案してほしいと依頼された
- AskUserQuestionで機能候補（編集／削除／最小文字数チェック／AI再生成／プレビュー／下書き保存）を提示し、「提出済み要約の編集」を最優先で実装することで合意
- 実装（`src/app/farm/FarmCharacters.tsx`）:
  - 要約表示カードに「✏️ 編集する」リンクを追加。押すと、提出時に使ったソース（貼り付け/文字起こし/メモ＋質問）ごとに元の入力内容（`summary_raw_input`・`summary_qa`）をフォームへ復元して再オープンする。文字起こし・メモ＋質問は編集後にAIで要約を再生成できる
  - `MeetingHistoryItem`型と`MEETING_HISTORY_SELECT`に、これまで取得していなかった`summary_raw_input`列を追加（編集時の復元に必要なため）
  - 実装中に発見・修正したバグ: 編集して再提出するたびにキャラの成長カウント（`onSummarySubmitted`）が重複加算される作りになっていたため、「その記録が初めて要約提出されるときだけ」加算されるよう修正（`historyItems`から編集前の`summary_submitted_at`の有無を見て判定）
  - `npm run build`成功、`npm run lint`は新規エラーなし
- 次にやること: 人間が実機で「編集する」から中身を直して再提出し、(1) 内容が正しく上書きされるか (2) 成長カウントが二重に増えていないか、を確認

### 2026-07-06 不具合修正 — RLS解消後、「肥料をあげる」が空レコードを量産する問題を修正
- ユーザーがSupabaseダッシュボードで`meetings`テーブルのRLSを確認・対応（Disable RLSを実施）した結果、insertが成功するようになったが、「肥料をあげるを押すと、内容ないのにどんどん保存されて行く」と報告
- 原因: `handleStartDirectFeed`がボタンを押した瞬間に（まだ要約の中身を入力する前に）空のmeetings行を先にinsertし、その後で要約フォームを開く設計になっていたため、RLSが直って書き込みが成功するようになった途端、押すたびに中身のない記録が量産される状態になっていた
- 修正（`src/app/farm/FarmCharacters.tsx`）: 「肥料をあげる」ボタンは要約入力フォームを開くだけにしてDB書き込みを行わないよう変更。実際の保存は「要約を提出する」を押した瞬間の1回のみとし、レコード作成と要約内容の保存を1つのinsertで同時に行うようにした（`handleSummarySubmit`を`meetingId: string | null`を受け取る形に一般化し、`null`のときは新規insert、既存IDのときは従来通りupdate）。フォームの中身（3タブ入力欄）は既存記録への追記用と共通化し、`renderSummaryFormFields`として1箇所にまとめて重複を排除
- `npm run build`成功。`npm run lint`で従来からある実験的ルール（react-hooks/refs）由来の警告が2件増えたが、実際にrefをrender中に読んでいるわけではなく（ボタン押下時のみ実行される関数内での参照）、既存の同種警告10件と同じく実害のない誤検知と判断し許容
- ユーザーの過去の誤操作で作られた中身のない「🌾 肥料やり（直接記録）」テストレコードは、ユーザー自身がSupabase Table Editorから削除する方針
- 次にやること: 人間が実機で「肥料をあげる」→フォーム入力→提出、および何も入力せずキャンセル、の両方を確認 → 問題なければ次のタスクへ
...
| 2026-07-06 | **【解決済み】`meetings`テーブルへのINSERTが全てRLSで弾かれていた**。「肥料をあげる」ボタンで「記録の作成に失敗しました」と報告があり調査したところ、`RequestForm.tsx`の「種をまく（新しいリクエストを送る）」が使っているのと全く同じ形のinsertでもanonキーから`new row violates row-level security policy for table "meetings"`で失敗することを確認した。アプリの中核機能である「種をまく」フロー自体がDBレベルで全滅している状態だった | ユーザーがSupabaseダッシュボードで`meetings`テーブルのRLSを確認し「Disable RLS」を実施して解決。ただしその後、RLSが直ったことで「肥料をあげる」が空レコードを量産するバグが発覚し別途修正（同日の該当セッション履歴を参照） |
