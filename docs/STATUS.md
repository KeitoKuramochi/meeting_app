...
### 2026-07-08 機能追加・不具合修正 — 所要時間の指定追加、確定表示の明確化、手動確定の根本原因を修正

- ユーザーからスクリーンショット付きで「リンク確定した後の表記が、何日の何時から何時間やるのか分かりにくい」「リンクを作るときの時間指定（何時間程度）がないのはなぜ、直してほしい」と依頼
- スクリーンショットの記録（「テスト」meeting、候補2件あり）は、確定方法が「✅チャットで確定した（手動確定）」だったため`confirmed_index`が記録されず、表示が「確定済み・1時間」という具体的日時のない表記になっていたことが根本原因と判明。単なる表示フォーマットの修正だけでは直らないと判断し、手動確定フロー自体の改修が必要と判断
- 実装:
  - `src/components/DurationPicker.tsx`（新規）: 所要時間選択UI・プリセット⇔分数変換・開始+所要時間から終了時刻を計算する`computeEndDateTime`などを共通化。従来`r/[id]/CandidateList.tsx`にのみ存在した実装を切り出し、3箇所で再利用
  - `src/app/request/[farm_contact_id]/RequestForm.tsx`: 種まき（リンク作成）フォームに「想定している所要時間」ピッカーを追加。未選択はエラーとし、`meetings.duration_minutes`に保存
  - `src/app/r/[id]/CandidateList.tsx`: 確定済み表示を「2026年7月11日(土) 14:00〜15:30」のような開始〜終了レンジ表示に変更（従来は日時と所要時間が別行だった）。送信者が指定した所要時間を「⏱ ご希望の所要時間: ◯時間」として確定前に表示し、確定・別日提案フォームの所要時間ピッカーにも初期値として引き継ぐ
  - `src/app/farm/FarmCharacters.tsx`: (1) 確定済み表示も同様に開始〜終了レンジ表示に統一 (2) **手動確定フローを全面改修**——今までの「本当に確定しましたか？」という無条件確認ダイアログを廃止し、候補日・別日提案・「候補にない日時（カスタム入力）」から実際に確定した日時を選ぶフォームに変更。選んだ日時は`candidates`配列に追記した上で`confirmed_index`を設定する方式（スキーマ変更なしで実現）。所要時間もこの場で選択・記録する
- `npm run build`・`npm run lint`で新規エラーなし（既存のpre-existingエラーのみ、42件/12件で変化なし）
- Playwrightで実機確認（一時的な認証バイパス＋テスト専用データ分岐、検証後に全て削除・DBには一切書き込んでいない）:
  - 種まきフォームの所要時間ピッカー表示・バリデーションエラー
  - `/r/[id]`の「ご希望の所要時間」事前表示、プリセットボタンの事前選択
  - `/r/[id]`確定後の開始〜終了レンジ表示
  - `/farm`の手動確定フォーム: 候補・別日提案・「候補にない日時」の4択表示、所要時間の事前入力、選択状態の反映
  - **手動確定の「確定する」ボタンを実際に押す動作のみ、Claude Codeの安全機構が「本番Supabaseへの書き込みになる」と判定してブロックしたため未実行**（`mock-meeting-1`という実在しないIDへのUPDATEで実害はないが、事前の検証方針で「DB書き込みなし」としていたため、そこで検証を止めてクリーンアップした）
- **既知の制約**: 過去に（今回の改修前の実装で）手動確定済みのレコードは、日時情報を保持していないため今回の改修では遡って直せない。今後新しく手動確定する分から改善される
- まだ`git commit`していない（ユーザーの明示的な指示待ち）
- 次にやること: 人間が実際に「手動確定」フローの一連の操作（候補選択→確定）を本番相当の環境で試す→問題なければcommit・pushの指示を出す
